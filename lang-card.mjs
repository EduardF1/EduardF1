// lang-card.mjs - generate a static SVG "most used languages" card for the profile
// README from real GitHub byte data (gh api, all repos incl. contribution forks,
// programming languages only). No third-party image host = never 503, never stale camo.
// Run: node lang-card.mjs > assets/lang-card.svg
import { execFileSync } from "node:child_process";

const langs = {};
const repos = execFileSync("gh", ["api", "users/EduardF1/repos", "--paginate", "--jq", ".[].name"], { encoding: "utf8" }).trim().split(/\r?\n/).filter(Boolean);
...
const rows = sorted.map(([name], i) => {
  ...
  return `<text x="${PAD}" y="${24 + i * LH}" font-family="Segoe UI, Arial, sans-serif" font-size="13" fill="#e6edf3">${name}</text>
<text x="${W - PAD}" y="${24 + i * LH}" text-anchor="end" font-family="Consolas, monospace" font-size="12" fill="#9aa7b4">${pct}%</text>
<rect x="${PAD}" y="${30 + i * LH}" width="${((sorted[i][1] / total) * (W - 2 * PAD)).toFixed(1)}" height="7" rx="3.5" fill="${color(name)}"/>`;
}).join("\n");
const H = 40 + sorted.length * LH;
console.log(`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" role="img" aria-label="Most used programming languages">
  <rect width="${W}" height="${H}" rx="12" fill="#0d1117" stroke="#26303b"/>
  <text x="${PAD}" y="-8" font-family="Segoe UI, Arial, sans-serif" font-size="10" fill="#9aa7b4" transform="translate(0,28)">MOST USED PROGRAMMING LANGUAGES (GitHub bytes, all repos incl. contribution forks)</text>
${rows}
</svg>`);
